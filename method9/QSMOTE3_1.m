function [ QSMOTEDateSet ] = QSMOTE3_1(  trainSet,majorClassNo,minorClassNo,K )
...
% 若集合與集合有相同元素,則合併此集合.
% 邊界少數類別與該群內插產生實力  %若群只有一點則此群點與最近點內插(最近的多數類別實力)
% 以siminSet當作SEED SET
% 若群內只有一點則找最近點內插(這點應該是多數類別)
...
ratio=round((majorClassInstanceCount-size(sminfSet,2))/size(siminSet,1));
if ratio>=2 
    ratio=ratio-1; 
end
